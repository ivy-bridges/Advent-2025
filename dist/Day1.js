...
function resolveRotation(angle) {
    if (angle >= 0) {
        return angle % 100;
    }
...
function zeroPasses(baseAngle, rotation) {
...
// part 2 solution 
function countZeroPasses(rotationList) {
    ...
    let rotationSteps = rotationList.reduce((acc, newRotation) => {
        const [numPasses, currentRotation] = acc; // and destructure it later
        //console.log(`Rotating ${newRotation} from ${currentRotation} to ${resolveRotation(currentRotation + newRotation)} (${zeroPasses(currentRotation, newRotation)})`)
        //console.log(`Passed through zero ${zeroPasses(currentRotation, newRotation)} times`)
        //console.log(`${numPasses} plus ${zeroPasses(currentRotation, newRotation)}`)
    ...
    // rotationSteps = [numPasses, finalRotation]
...
function breakRotation(rotation) {
    if (rotation >= 0) {
        return Array(rotation).fill(1);
    }
    return Array(-1 * rotation).fill(-1);
}
// let's wrap it up
// takes the full input and prints the result
export default function day1(inputString) {
