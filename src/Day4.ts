...
function parseGrid(gridText : string) : boolean[][] {
    let gridRows = gridText.split('\n').map(r => r.trim().split(''))
    return gridRows.map(r => r.map(i => (i == "@")));
}

// using row-then-column ordering
function indexByTuple<T>([x, y] : [number, number], posGrid : T[][]) : T {
    return posGrid[y][x]
}

// given some index position, grabs the <8 surrounding items
function getNeighbors<T>([x,y] : [number, number], posGrid : T[][]) {
    const neighborLocations : [number,number][]  = ([ // get prospective neighbors
        [x-1, y-1], [x, y-1], [x+1, y-1], 
        [x-1, y],             [x+1, y],      // v (typescript widens to number[][] without assertion)
        [x-1, y+1], [x, y+1], [x+1,y+1]] as [number,number][]).filter(([xPos, yPos]) => // and check whether within bounds
...
function countOccupiedNeighbors([x,y] : [number, number], posGrid : boolean[][]) : number {
...
function countAccessibleRolls(posGrid : boolean[][]) : number {
    let numRolls = 0;
...
function countTotalRolls(posGrid : boolean[][]) : number {
...
function findAccessibleRolls(posGrid : boolean[][]) : [number,number][] {
    let accessibleLocations : [number, number][] = [];
    for (let y = 0; y < posGrid.length; y++) {
        for (let x = 0; x < posGrid[y].length; x++) {
            if (posGrid[y][x] && countOccupiedNeighbors([x,y], posGrid) < 4) {
...
function trimAccessibleRolls(posGrid : boolean[][]) : boolean[][] {
...
function countMaxRemovable(posGrid : boolean[][]) : number {
...
export default function day4(inputString : string) : void {
