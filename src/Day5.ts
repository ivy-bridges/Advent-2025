...
// fresh ingredient id ranges are given as an upper and lower bound
...
function grabRanges(input : string) : range[] {
    return input.split('\n') // split by lines
...
function grabIngredients(input : string) : number[] {
    return input.split('\n') // split by lines
...
function inRange(ingredient : number, idRange : range) : boolean {
    let [lower, upper] = idRange
    return (ingredient >= lower) && (ingredient <= upper)
}

// 
function isFresh(ingredient : number, idRanges : [number, number][]) : boolean {
...
function updateRanges(staleRanges : range[], freshRange : range) : range[] {
...
export default function day5(inputString : string) {
